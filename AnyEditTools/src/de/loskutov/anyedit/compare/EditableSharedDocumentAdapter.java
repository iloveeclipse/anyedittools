...
/**
 * A shared document adapter that tracks whether the element is connected to a shared document and
 * whether the contents have been flushed from a compare viewer. When contents are flushed, this
 * adapter will connect to the document provider to ensure that the changes are not lost (see
 * {@link #hasBufferedContents()}). In order to avoid a leak, the buffer must either be saved (see
 * {@link #saveDocument(IEditorInput, boolean, IProgressMonitor)}) or released (see
 * {@link #releaseBuffer()}).
 * <p>
 * This adapter must have a one-to-one correspondence to a typed element.
 */
public class EditableSharedDocumentAdapter extends SharedDocumentAdapter implements
IElementStateListener {
    ...
    /**
     * Interface that provides this adapter with the state of the typed element and supports call
     * backs to the element when the adapter state changes.
     */
    public static class SharedDocumentAdapterListener {
        ...
        /**
         * Method that is invoked when the adapter connects to the document provider. This method is
         * only invoked when the adapter first connects to the document.
         */
        ...
        /**
         * Method that is invoked when the adapter disconnects from the document provider. This
         * method is only invoked when the adapter no longer has any connection to the document
         * provider.
         */
     ...
     * @param overwrite
     *            indicates whether overwrite should be performed while saving the given element if
     *            necessary
     * @param monitor
     *            a progress monitor
    ...
    /**
     * Return whether the adapter has buffered contents. The adapter buffers contents by connecting
     * to the document through the document provider. This means that the adapter must be
     * disconnected either by saving or discarding the buffer.
     *
